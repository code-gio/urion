<script lang="ts">
	...
	let selectedUser = $state<{ id: string; email: string; full_name: string | null; display_name: string | null; avatar_url: string | null; isMember?: boolean } | null>(null);
	let searchQuery = $state('');
	let searchResults = $state<Array<{ id: string; email: string; full_name: string | null; display_name: string | null; avatar_url: string | null; isMember?: boolean }>>([]);
	...
	const canManageMembers = $derived(userRole === 'owner' || userRole === 'admin');
	...
	async function searchUsers() {
		...
		isSearching = true;
		try {
			const response = await fetch(`/api/users/search?q=${encodeURIComponent(searchQuery)}&workspaceId=${workspace.id}`);
			if (response.ok) {
	...
	async function updateMemberRole() {
		...
		try {
			const response = await fetch(`/api/workspaces/${workspace.id}/members/${editingMember.id}`, {
				method: 'PATCH',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	async function deleteMember() {
		...
		try {
			const response = await fetch(`/api/workspaces/${workspace.id}/members/${deletingMember.id}`, {
				method: 'DELETE'
			});
	...
	async function cancelInvite(inviteId: string) {
		...
		try {
			const response = await fetch(`/api/workspaces/${workspace.id}/members/${inviteId}`, {
				method: 'DELETE'
			});
	...
	{#if data.workspaceOwner}
		<Card class="mb-8">
			<CardHeader>
				...
				<CardDescription>The creator and owner of this workspace</CardDescription>
			</CardHeader>
					...
					<div class="flex items-center gap-3">
						{#if data.workspaceOwner.avatar_url}
							<img
								...
								alt={data.workspaceOwner.display_name || data.workspaceOwner.full_name || 'Owner'}
								class="w-10 h-10 rounded-full"
							/>
						{:else}
							<div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
								<span class="text-sm font-medium">
									{(data.workspaceOwner.display_name || data.workspaceOwner.full_name || data.workspaceOwner.email || 'O').charAt(0).toUpperCase()}
								</span>
							</div>
						{/if}
						<div>
							...
							<p class="text-sm text-muted-foreground">{data.workspaceOwner.email}</p>
						</div>
					</div>
...
<Dialog open={editingMember !== null} onOpenChange={(open) => !open && (editingMember = null)}>
	<DialogContent>
		<DialogHeader>
			<DialogTitle>Change Member Role</DialogTitle>
			<DialogDescription>
				Update the role for {editingMember ? getMemberDisplayName(editingMember) : ''}
			</DialogDescription>
		</DialogHeader>
		<div class="space-y-4 py-4">
			<div class="space-y-2">
				<Label for="edit-member-role">Role</Label>
				<Select.Root type="single" bind:value={editingRole} disabled={isUpdatingRole}>
		...
		<DialogFooter>
			<Button variant="outline" onclick={() => (editingMember = null)} disabled={isUpdatingRole}>
				Cancel
			</Button>
...
<Dialog open={showAddDialog} onOpenChange={(open) => !open && (showAddDialog = false)}>
	<DialogContent>
		<DialogHeader>
		...
		<div class="space-y-4 py-4">
			<div class="space-y-2">
				...
				<div class="relative">
					...
					/>
					{#if selectedUser}
						<div class="mt-2 p-3 bg-muted rounded-md flex items-center justify-between">
							<div class="flex items-center gap-3">
								{#if selectedUser.avatar_url}
									<img
										src={selectedUser.avatar_url}
										alt={selectedUser.display_name || selectedUser.full_name || selectedUser.email}
										class="w-8 h-8 rounded-full"
									/>
								{:else}
									<div class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
										<span class="text-xs font-medium">
											{(selectedUser.display_name || selectedUser.full_name || selectedUser.email).charAt(0).toUpperCase()}
										</span>
									</div>
								{/if}
								<div>
					...
					{:else if searchResults.length > 0}
						<div class="absolute z-50 mt-1 w-full bg-popover border rounded-md shadow-lg max-h-60 overflow-auto">
							{#each searchResults as user}
								<button
									type="button"
									class="w-full p-3 hover:bg-accent flex items-center gap-3 text-left {user.isMember ? 'opacity-60 cursor-not-allowed' : ''}"
								...
								>
									...
									{#if user.isMember}
										<Badge variant="secondary" class="text-xs">Already member</Badge>
									{/if}
								</button>
							{/each}
						</div>
					{:else if isSearching}
						<div class="absolute z-50 mt-1 w-full bg-popover border rounded-md shadow-lg p-3">
		...
		<DialogFooter>
			<Button variant="outline" onclick={() => (showAddDialog = false)} disabled={isAdding}>
				Cancel
			</Button>
...
<Dialog open={deletingMember !== null} onOpenChange={(open) => !open && (deletingMember = null)}>
	<DialogContent>
		<DialogHeader>
			<DialogTitle>Remove Member</DialogTitle>
			<DialogDescription>
				Are you sure you want to remove {deletingMember ? getMemberDisplayName(deletingMember) : ''}
				from this workspace? This action cannot be undone.
			</DialogDescription>
		</DialogHeader>
		<DialogFooter>
			<Button variant="outline" onclick={() => (deletingMember = null)} disabled={isDeleting}>
				Cancel
			</Button>
